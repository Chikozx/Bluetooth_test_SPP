...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_CLOSE_EVT:
        ...
        ESP_LOGI(SPP_TAG, "SPP connection closed, restarting server...");
        esp_spp_stop_srv();
        esp_spp_start_srv(sec_mask, role_slave, 0, SPP_SERVER_NAME);
        break;
    ...
    case ESP_SPP_DATA_IND_EVT:
        ...
        if (param->data_ind.len < 128)
        {
            ...
            receive_control(param->data_ind.data, param->data_ind.len);
        }
#else
...
esp_err_t esp_bluetooth_stack_init(void){
    ...
    //Initialize & Enable BT Controller with default config 
    ...
    //Initialize & enable BT Host (Bluedroid) with default config
    ...
    //Register Callback function for GAP and SPP
